...
const GuardianCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(form.estudiantes);
    if (form.estudiantes.length === 0 || form.estudiantes.some(s => !s.parentesco_id)) {
      Swal.fire("Atención", "Debe asignar al menos un estudiante con su parentesco", "warning");
      return;
    }

    try {
      setLoading(true);
      await axiosInstance.post("/guardians/create", {
        ...form,
      ...
      Swal.fire(
        "Error",
        error.response?.data?.error || "Error al crear encargado",
        "error"
      );
    } finally {
      setLoading(false);
