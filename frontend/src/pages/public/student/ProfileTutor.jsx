...
export default function ProfileTutor({ user }) {
  // Nota: En un caso real, estos datos vendrían del localStorage o una API
const tutor = user?.perfiles?.tutor ?? {};
const [selectedAvatar, setSelectedAvatar] = useState(null);
const [selectedFrame, setSelectedFrame] = useState(null);

const uuid = tutor?.uuid;
console.log(user);
...
const updateLocalUser = (type, value) => {
  ...
  if (!storedUser?.perfiles?.tutor) return;

  if (type === "avatar") {
    storedUser.perfiles.tutor.avatar = value;
  }

  if (type === "marco") {
    storedUser.perfiles.tutor.marco = value;
  }
...
useEffect(() => {
  if (tutor?.avatar?.uuid) {
    setSelectedAvatar(tutor.avatar.uuid);
  }

  if (tutor?.marco?.uuid) {
    setSelectedFrame(tutor.marco.uuid);
  }
}, [tutor]);
// separar resultados
...
const currentAvatar =
  avatars.find((a) => a.uuid === selectedAvatar) || tutor?.avatar;

const currentFrame =
  frames.find((f) => f.uuid === selectedFrame) || tutor?.marco;
if (isLoading) {
  return <Loader text="Cargando perfil..." />;
}
    return (
        <>
        <Hero title={tutor.alias!=''?tutor.alias:user.nombre}/>
  ...
  currentAvatar?.imagen
    ...
    />
                ...
                <div className="profile-summary-body">
                    ...
                    tutor de CETAV con interés en producción audiovisual y creación de contenido.
                    </p>
        ...
        <div className="profile-video-thumb">
            ...
            <span className="profile-video-avatar">
                <img 
                    src={
                        tutor?.avatar?.imagen
                        ? `${API_URL_IMG}${tutor.avatar.imagen}`
                        : `${URL_IMG}images/assets/avatar.png`
                    } 
                    alt="Avatar" 
                />
